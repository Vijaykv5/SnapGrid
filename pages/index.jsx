...
export default function Home() {
  ...
  let fetchImages = async () => {
    try {
      if (searchInput.current?.value) { // fixed problem 1
  ...
  const handleClick = (e) => { // fixed problem 2
    e.preventDefault();
    // console.log(searchInput.current.value);
    const titleArray = links.map((obj) => 
    obj.title
  )
  if (searchInput.current && titleArray.indexOf(searchInput.current.value) === -1) // fixed problem 3
  {
    setBannerImage(null)
  }
    images != null ? (
